            ...
            {categories.map((category) => (
              <button
                ...
                className={`px-6 py-3 rounded-full font-heading font-semibold transition-all duration-300 ${selectedCategory === category
                  ...
                  : 'text-light-text-primary dark:text-text-primary border border-white/5 dark:border-white/10 hover:border-white/50 backdrop-blur-sm'
                  }`}
              >
                {category}
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredCertificates.map((certificate, index) => (
              <motion.div
                ...
                className="group relative rounded-2xl p-6 border border-white/5 dark:border-white/10 hover:border-primary transition-all duration-300 hover:shadow-xl backdrop-blur-sm bg-transparent"
              >
                <GlowingEffect
                  spread={40}
                  glow={true}
